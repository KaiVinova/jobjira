...
class MultiSelectionVC: UIViewController {
    ...
    @IBAction func doneBtnAction(_ sender: Any) {
        if selectedArray.count>0 {
            ...
            let alert = UIAlertController(title: AppName, message: "Please choose from options to proceed", preferredStyle: UIAlertControllerStyle.alert)
...
extension MultiSelectionVC:UITableViewDataSource{
    ...
    func jobTypeSelectionBtnAction(_ sender:UIButton){
        ...
        if sender.isSelected{
            if sender.tag == optionsArray.count-1 {
                ...
                if selectedArray.count>0 {
                    if selectedArray.count == 1 && selectedArray[0]["optionText"] as! String == "All"{
                        selectedArray.removeAll()
                        selectedArray.append(optionsArray[sender.tag])
                        jobTypeTable.reloadData()
                    }
