...
@objc protocol ExpectedSalaryDelegate
{
    @objc optional func callBackWithDict(data:[String:AnyObject])
}
class ExpectedSalaryVC: UIViewController
{
...
//MARK:- View LifeCycle
    ...
    override func viewDidLoad()
    {
        ...
        if (SignUpMetaData.sharedInstance.expectedSalaryDict["obQuestionID"]) != nil
        {
            dataDict = SignUpMetaData.sharedInstance.expectedSalaryDict
        }
...
//MARK:- Method for SetupData on View
    ...
    func setUpViewBackEndData()
    {
        ...
        fresherLabel.text = dataDict["questionTexts"]?["i_am_flexible"] as? String
        currency = dataDict["questionTexts"]?["currency"] as? String
        salaryeLabel.text = "Expected Salary"
        if SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"] as? [String:AnyObject] != nil
        {
            if SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salaryTypeID"] as? String == "3"
            {
                fresherBtn.isSelected = true
            }
            else if SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salaryTypeID"] as? String == "1"
            {
            ...
            else if SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salaryTypeID"] as? String == "2"
            {
            ...
            let salary = "\(currency!) \(SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salary"] as! String)"
...
//MARK:- Month Button Action
    ...
    @IBAction func monthBtnAction(_ sender: Any)
    {
        ...
        {
            let alert = UIAlertController(title: AppName, message: "Please uncheck - \"I'm flexible, I'll take whatever I'll get.\" checkbox to specify your expected salary", preferredStyle: UIAlertControllerStyle.alert)
...
//MARK:- Done Button Action
    ...
    @IBAction func doneBtnAction(_ sender: Any)
    {
        ...
        {
            var tempDict = SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"] as! [String:AnyObject]
            ...
            SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"] = tempDict as AnyObject?
            if editFlag == true
            {
                SignUpMetaData.sharedInstance.userInfoDict["expectedSalaryTypeID"] = SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salaryTypeID"] as AnyObject
                SignUpMetaData.sharedInstance.userInfoDict["expectedSalary"] = SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salary"] as AnyObject
                let _ = navigationController?.popViewController(animated: true)
            }
            else
            {
        ...
        {
            let alert = UIAlertController(title: AppName, message: "Please specify your expected salary to proceed", preferredStyle: UIAlertControllerStyle.alert)
            let action =  UIAlertAction(title: AlertOk, style: UIAlertActionStyle.default, handler: { (action) -> Void in
            })
            alert.addAction(action)
            self.present(alert, animated: true, completion: nil)
...
// MARK: Add item detail view Method
    ...
    {
        let popup = ExpectedSalaryView.instansiateFromNib()
        popup.expectedSalaryDelegate = self
        popup.frame = CGRect(x: 0, y: 0, width: self.view.frame.size.width, height: self.view.frame.size.height)
        popup.currency = currency
        if SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"] != nil
        {
            popup.selectedText = SignUpMetaData.sharedInstance.expectedSalarySelectedDict["answer"]?["salary"] as! String
        }
...
//MARK:- Fresher Button Action
    ...
    @IBAction func fresherBtnAction(_ sender: Any)
    {
        ...
        {
            SignUpMetaData.sharedInstance.expectedSalarySelectedDict = [String:AnyObject]()
        }
    }
}

//MARK:- Extension ExpectedSalaryDelegate

extension ExpectedSalaryVC:ExpectedSalaryDelegate
{
