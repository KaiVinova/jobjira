...
class DegreeView: UIView {
    ...
    var degreeDelegate:DegreeDelegate?
    ...
    class func instansiateFromNib() -> DegreeView{
        return Bundle.main.loadNibNamed("Views", owner: self, options: nil)! [15] as! DegreeView
    }
    ...
    @IBAction func doneBtnAction(_ sender: Any) {
        if salaryEntryTxt.text != "" {
            ...
            tempDict["degree"] = salaryEntryTxt.text as AnyObject?
            degreeDelegate?.callBackWithDegreeDict!(data: tempDict)
            self.endEditing(true)
            self.hidePopUp()
        }
        else{
            let alert = UIAlertController(title: AppName, message: "Enter your last degree.", preferredStyle: UIAlertControllerStyle.alert)
...
extension DegreeView: UITextFieldDelegate{
