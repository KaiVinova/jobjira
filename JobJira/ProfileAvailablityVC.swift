...
class ProfileAvailablityVC: UIViewController
{
...
//MARK:- View LifeCycle
    ...
    override func viewDidLoad()
    {
        ...
        {
            dataDict = SignUpMetaData.sharedInstance.profileAvailabilityDict
        }
        if editFlag == true
        {
...
//MARK:- Method for SetupData on View
    ...
    func setUpViewBackEndData()
    {
        ...
        let questionOptions = dataDict["questionOptions"]?["profileAvailability"] as? [AnyObject]
        optionsArray = questionOptions!
        if SignUpMetaData.sharedInstance.profileAvailabilitySelectedArray.count>0
        {
            selectedArray = SignUpMetaData.sharedInstance.profileAvailabilitySelectedArray as [AnyObject]
        }
    }
...
//MARK:- Done Button Action
    ...
    @IBAction func doneBtnAction(_ sender: Any)
    {
        ...
        {
            SignUpMetaData.sharedInstance.profileAvailabilitySelectedArray = selectedArray as! [[String : AnyObject]]
            if editFlag == true
            {
                SignUpMetaData.sharedInstance.userInfoDict["profileAvailabilityType"] = SignUpMetaData.sharedInstance.profileAvailabilitySelectedArray[0]["profileAvailabilityType"] as AnyObject
                let _ = navigationController?.popViewController(animated: true)
            }
            else
            {
        ...
        {
            let alert = UIAlertController(title: AppName, message: "Please mark your profile availability to proceed", preferredStyle: UIAlertControllerStyle.alert)
...
extension ProfileAvailablityVC:UITableViewDataSource
{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        cell.jobTypeLabel.text = optionsArray[indexPath.row]["profileAvailabilityType"] as? String
        cell.jobSubTypeLabel.text = optionsArray[indexPath.row]["buttonText"] as? String
        ...
        {
            if element["profileAvailabilityTypeID"] as! String == optionsArray[indexPath.row]["profileAvailabilityTypeID"] as! String
            {
...
//MARK:- Job tyope selection Button Action
